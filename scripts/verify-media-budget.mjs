...
const validPath = path => /^media\/(sfx|images)\/[a-z0-9]+(?:-[a-z0-9]+)*\.(webp|avif|ogg|mp3|m4a)$/.test(path ?? '');
export function checkMediaAssets(assets) {
  ...
  for (const asset of assets) {
    ...
    if (kind === 'image' ? !/^media\/images\/.*\.(avif|webp)$/.test(path) : !/^media\/sfx\/.*\.(ogg|mp3|m4a)$/.test(path)) failures.push(`格式与用途不符：${path}`);
...
export async function verifyMedia(root, target = null) {
  ...
  if (!Array.isArray(assets) || assets.some(asset => !validPath(asset.path))) throw new Error('媒体清单必须使用 public/media 内的合法路径');
  ...
  for (const base of ['public', ...(target ? [target] : [])]) {
    ...
    if (base !== 'public') for (const asset of measured) if ((await stat(resolve(root, base, asset.path))).size !== asset.bytes) result.failures.push(`产物资源不一致：${asset.path}`);
  }
