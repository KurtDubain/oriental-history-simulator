...
async function openFreshWorld(page, seed = null) {
  ...
  await page.waitForFunction(() => JSON.parse(window.render_game_to_text()).productVersion === '1.0.1');
}
...
try {
  ...
  assert.equal(initial.productVersion, '1.0.1');
  ...
  assert.equal(reloaded.productVersion, '1.0.1');
  ...
  assert.equal((await snapshot(mobilePage)).productVersion, '1.0.1');
