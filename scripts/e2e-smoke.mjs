...
async function openFreshWorld(page, seed = null) {
  ...
  await page.waitForFunction(() => JSON.parse(window.render_game_to_text()).productVersion === '1.1.2');
}
...
try {
  ...
  assert.equal(initial.productVersion, '1.1.2');
  ...
  assert.equal(reloaded.productVersion, '1.1.2');
  ...
  assert.equal((await snapshot(mobilePage)).productVersion, '1.1.2');
