...
try{for(const[label,width,height]of[['desktop',1440,900],['mobile',390,844]]){
 ...
 await page.locator('input[type=file]').setInputFiles('output/review-v1.29.24/late/t336.portable.json');await page.locator('.world-map__canvas').waitFor();await page.locator('.world-start').waitFor({state:'hidden'});await page.locator('.observer-toast').waitFor({state:'hidden'});assert.equal((await audio()).starts.length,prior,'import may not broadcast old history');
