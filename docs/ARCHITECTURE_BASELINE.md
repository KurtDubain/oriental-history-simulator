...
> 建立于 2026-08-27，v1.27.0 聚焦减法收口于 2026-09-06，命令：`npm run test:audit:architecture`

## 当前规模

- `src` 下生产 TypeScript / TSX：171 个文件，61,589 行（测试排除）。v1.27.0 删除整套音频 owner、邀请层和无自然实例的公开拒令分支，以连续处境替换旧响应 / 战后分档，并让人物经历与地图布局继续保持只读投影。
- 相对模块依赖：676 条，其中 439 条 runtime、237 条 type-only；运行时环与跨层违例均为 0，类型总图仍只有既有的 12 模块契约环，正好落在 12/12 的不增长预算内。
- 当前热点：`engine.ts` 3,118/3,120 行、`invariants.ts` 2,665/2,665 行、`App.tsx` 2,205/2,300 行（实际/门禁）。
- 其次为 `v02.ts` 2,501 行、`v03-ocean.ts` 2,460 行、`agency/decision.ts` 2,042 行、`Inspector.tsx` 1,364 行与 `map-renderer.ts` 1,136 行。`observer-leads.ts` 为 396/400 行，`WorldMap.tsx` 为 1,053/1,100 行，`view/adapters.ts` 为 52/100 行。
...
- v1.27.0 没有升级 schema 5、没有增加 `WorldState` 顶层字段，也没有增加季度阶段。固定 seed 的新历史会因响应和战后裁决改变，但同版本重放、存读档续推的 world hash、Fact digest 与 History digest 必须完全一致。
