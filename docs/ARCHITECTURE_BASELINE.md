...
> 建立于 2026-08-27，v1.29.3 普通玩家阅读收口于 2026-09-08，命令：`npm run test:audit:architecture`

## 当前规模

2026-09-09 悲剧英雄与战争平衡收口（本地v1.29.7，未发布）：相对19e25f4/v1.29.6，生产171文件不变，62,281→62,365行（+84）；App2,217、invariants2,665、observer-leads399不变，engine3,102→3,105、WorldMap1,073→1,077。依赖683（runtime445/type238）、runtime环0、类型债务12不变。复用既有人物只读缓存与事实、军令、健康和再战边界；不新增schema、WorldState字段、季度阶段或依赖。两版全部JS gzip427,198→429,094，CSS39,661不变，**JS预算仍失败且增加1,896 bytes**；原424,960预算与本轮419,840目标均未满足，不修改预算。仅调整现有simulation-support分块，未排除异步JS计数。709单测与架构门通过不能代表发布通过；人物命运专项、包体及冷开目标未完成，详见 `output/tragedy-v1.29.7/REPORT.md` 和 progress.md。本段是当前实测值，下方保留历史基线。

2026-09-09 人物叙事发现修订（本地v1.29.5，未发布）：相对 `ed60782`，生产171文件不变，62,128→62,127行（-1）；App2,220、engine3,098、invariants2,665、observer-leads399均不变。依赖679→680（runtime441→442、type-only238），复用既有可执行军令查询；运行时环0、类型债务12不变。个人/参赛JS gzip424,949→424,958 bytes（+9），CSS39,653不变；预算没有提高。人物回看读取现有冷档预览、actor索引及带来源履历，完整人生只在开档时提炼，不新增WorldState、schema、季度阶段、包依赖或持久化owner。84项相关测试、两版构建及架构门通过；全量与E2E仍有真实失败，见 [本轮报告](HISTORY_DISCOVERY_REVIEW.md)，不得标作全绿发布。

2026-09-08 历史合理性修订（本地v1.29.4，未发布）：相对HEAD `fe36818`，生产171文件不变，62,142→62,128行（-14）；App2,220不变、engine3,100→3,098、invariants2,665、observer-leads399。679依赖（runtime441、type-only238）、运行时环0、类型环12均不变。个人/参赛JS gzip424,942→424,949 bytes（+7），CSS39,653不变；预算424,960/40,960不提高。既有季度内物流路径缓存复用同源最短路，季度后丢弃，不进入WorldState。规则修订复用军令、Shipment、Fact、背景人口、职位与人物关系，无新增schema、季度阶段或依赖。完整测试与尚未通过专项见 [历史合理性报告](HISTORICAL_CONTINUITY_REVIEW.md)，不能把架构门通过等同于发布验收通过。

- `src` 下生产 TypeScript / TSX：171 个文件，62,093 行（测试排除，相对 v1.29.2 增加 112 行、文件数不变）。人物事实去重、首战判断、选角与默认继承证据都复用现有只读投影；停表回执复用 `ObserverPauseMatch`，目录折叠复用现有展开状态，陆地落点复用已有有界采样并校验实际海岸，没有新增状态 owner、页面、schema 或模拟阶段。
- 相对模块依赖：679 条，其中 441 条 runtime、238 条 type-only；运行时环与跨层违例均为 0，类型总图仍只有既有的 12 模块契约环，正好落在 12/12 的不增长预算内。
- 当前热点：`engine.ts` 3,118/3,120 行、`invariants.ts` 2,665/2,665 行、`App.tsx` 2,220/2,300 行（实际/门禁）。
- 其次为 `v02.ts` 2,501 行、`v03-ocean.ts` 2,460 行、`agency/decision.ts` 2,042 行、`Inspector.tsx` 1,364 行与 `map-renderer.ts` 1,305 行。`observer-leads.ts` 为 399/400 行，`WorldMap.tsx` 为 1,073/1,100 行，`view/adapters.ts` 为 52/100 行。
- v1.29.3 个人/参赛包：JS gzip 423,730 bytes、CSS gzip 39,653 bytes；v1.29.2 为 421,574 / 39,506，分别增加 2,156 / 147 bytes。门禁仍为 424,960 / 40,960，不提高预算。`src/sim/**` 零改动，观察交互不改变世界、事实和存档正文。
...
- runtime 依赖图没有循环，也没有 `sim/maps → App/components/view/infra` 的跨层回写。
...
- 出征响应只在组建行营时运行：从与主将、所在地或战线确有关系的人中计算彼此竞争的响应 / 留守动机；同派不自动整派随征，只有真正响应者进入编队，普通留守不生成事实。10 个种子各 64 季始终没有自然公开拒令，因此其 outcome、Fact、史册、关系后果与 UI 分支已整体删除，不保留受控剧情。
- 战后命运只裁决 Battle Fact 中真实参战的人；风险连续读取本部损失、胜负、耗尽、军中位置、健康、谨慎、统率与撤退处境，仅在连续风险允许时使用确定性 keyed randomness。负伤沿用 `health` 并立即退营，恢复期限由同一伤情 Fact 连续推导；战死沿用统一死亡入口。
- 人物故事不进入 `WorldState`，不缓存阶段和分数，也不强制起点、得势、转折与近况。它按根 Fact 合并同一战役的会战、伤亡、退营与任免清理，保留 1～3 段可反链的“关键经历”；死者末段固定为有来源的“结局”。
- v1.27.1 没有升级 schema 5、没有增加 `WorldState` 顶层字段，也没有增加季度阶段；战争人物展开、局势证据与人话摘要都是读取现有事实的纯投影，固定 seed 的 world hash、Fact digest 与 History digest 不变。
