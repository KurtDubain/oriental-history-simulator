...
> 任务状态：Phase A 已完成；Phase B 已交付三类权威 Situation、事实型结案层与 B10 局势全卷，B09 数值化起止快照因权威字段缺失继续保留；Phase C 的 C01～C13 已完成 Situation-first 观察循环、人物记忆、跨季盘算，并交付“争取支持 → 请领军令 → 换帅/安抚/削权”的首段权威人物链。v1.5.0 已完成 EMB08 与 ARC03 首段：入世视角可精确续读，换人不刷新待办，人物离世自动回到观察并生成可追溯的生平收束；副将入世队列适配已离开总决策文件。下一产品入口为地方官赈济/减赋的小纵切，EMB06 通用行动统一仍待后续。
...
- [ ] **EMB05 — 身份专属行动**：副将首个纵切已交付，可随军令计划进度亲自联络本军将校、请求主帅/主君/家主背书或正式请领独立军令；地方官、朝臣、家主与君主仍按身份逐个交付，不一次铺开。
- [ ] **EMB06 — 与 AI 共用 Resolver**：副将身份行动已与 AI 共用 `resolveSupportAction` / `resolveIntent`，同队列排序且无玩家加成；三项通用行动仍使用入世 Resolver，待有对应 AI 行为后再收口整个任务。
- [x] **EMB07 — 结果进入历史**：通用与副将身份行动均产生唯一提交/结果 Fact；副将的观察者结果只反链领域 Fact，不重复认领状态差量，实际回应继续进入 Chronicle、PersonalMemory、人物传记、任命与权势账。
...
26. [ ] 入世身份扩展：副将与 EMB08 已交付；下一个只做地方官“赈济或减赋”小纵切，复用地方粮财压力、官职权限、国家财政、民怨/人口后果与具体史事。EMB06 持续作为同类 AI/玩家裁决一致性门禁，不一次铺开所有身份动作。
...
v1.3.0 的 P0 演示目标已经达成：玩家可随时入世任一存活人物，每季定下一项经营关系、争取机会或表明立场的行动；推进后生成唯一提交/结果 Fact，并把具体回应送回人物记忆、传记、关系和权势账。v1.4.0 继续完成副将首个身份纵切；v1.5.0 已完成 ARC03 首段拆分与 EMB08 生命周期/观察视角恢复。下一段选择地方官的赈济或减赋作为第二个小纵切，不扩张为全量角色扮演菜单。
