...
> 任务状态：Phase A 已完成；Phase B 已交付三类权威 Situation、事实型结案层与 B10 局势全卷，B09 数值化起止快照因权威字段缺失继续保留；Phase C 的 C01～C11 已完成 Situation-first 观察循环、人物记忆与跨季盘算，并让副将请领独立军令成为第一条权威行动链；当前先完成 NAR01～NAR04 的具体史事投影，再以 ARC02/POL01 为边界进入 C12/C13，随后交付 EMB01～EMB04 的轻量“人物入世”纵切
...
22. [ ] 人物入世首个纵切：完成 EMB01～EMB04，支持自由代入/离开、每季一次决定及关系/机会/立场三类行动，并复用 C12/C13 已交付的 Intent/Resolver。
23. [ ] 政治可见性纵切：完成 POL02～POL05，交付稳定派系、朝堂席位图、首都政治脉搏与有证据的地图权力根基；POL06～POL08 随 B05 和后续政治行为持续接入。
24. [ ] 入世身份扩展：按角色身份逐个完成 EMB05，EMB06～EMB08 持续作为裁决一致性、历史记录、生命周期、存档和防滥用门禁，不一次铺开所有身份动作。
25. [ ] C14/C15 前置门：完成 ARC03/ARC05 的第一轮拆分，确保权力基础、叛乱和后续战争系统不会继续堆入 App Shell 或总校验文件。
26. [ ] 地图与档案扩展前置门：完成 ARC04/ARC06；ARC07 持续作为每次拆分的行为保持门，ARC08 仅在性能证据满足时启动。
