...
> 任务状态：Phase A 已完成；Phase B 已交付三类权威 Situation、事实型结案层与 B10 局势全卷，B09 数值化起止快照因权威字段缺失继续保留；Phase C 的 C01～C13 已完成 Situation-first 观察循环、人物记忆、跨季盘算，并交付“争取支持 → 请领军令 → 换帅/安抚/削权”的首段权威人物链。INF01 已以 v1.0.1 交付更新闭环，v1.2.0 已把 NAR01～NAR04 与 POL01 接回同一条人物权力链。LOOP01 总任务仍待 EMB 入世共用同一 Intent/Resolver 后关闭
...
- [ ] **EMB01 — 观察者/入世模式契约**：人物档案可随时“以此人入世”，也可随时离开或换人；当前代入人物属于 observer metadata，不进入 `WorldState`、导出世界或 hash，单纯进入、退出和查看不得改变模拟结果。
- [ ] **EMB02 — 每季一次决定**：全世界每季最多接受一项玩家代入行动，离开、换人、读档界面切换均不刷新次数；是否已用由当季权威提交 Fact 确认，不依赖易丢失的 React/localStorage 标记。
- [ ] **EMB03 — 情境行动投影**：当前人物只看到 2～4 个由身份、职位、欲望、关系、资源和环境支持的行动；无自由文本、无固定全量菜单，不可行选项说明具体缺少的职位、对象或资源。
- [ ] **EMB04 — 首批三类行动**：先交付“经营关系”“争取机会”“表明支持/反对”三类通用行动；每项必须有明确目标人物/职位/议程、成本、主要阻碍和推进后的观察信号。
- [ ] **EMB05 — 身份专属行动**：在通用行动稳定后，逐步加入将领的整军/请战/请援、地方官的赈济/减赋请求、朝臣的上书/举荐、家主的族内支持、君主的召见/安抚/任命/削权；每类先做一个完整纵切，不一次铺开。
- [ ] **EMB06 — 与 AI 共用 Resolver**：玩家只选择人物想尝试的事，提交后与 AI Intent 使用同一权限、资源、关系、风险和冷却规则；`source=player_embodied` 只用于追溯，不提供成功率、数值或优先级加成，也不得从 UI 直接修改领域状态。
- [ ] **EMB07 — 结果进入历史**：提交、获准、遭拒、暂缓、失败和直接后果写入 typed Fact；重要结果进入 Chronicle、PersonalMemory 和人物传记，并按 NAR 契约具体说明谁做了什么、对方如何回应和状态怎样改变。
- [ ] **EMB08 — 生命周期、存档与防滥用审计**：人物死亡或失去世界载体时自动返回观察者并展示生平收束；世界存档保留已经发生的玩家行动，观察视角单独恢复。审计换人不刷新、同 seed/同操作确定性、无权限行动为零、移动端 44px 入口和代入前后普通 AI 连续运行。
...
22. [ ] 统一人物权力纵切：C12/C13、NAR01～NAR04 与 POL01 已连通支持、请令、机构回应、具体场面和权势结果账；下一步由 EMB01～EMB04 共用同一 Intent/Resolver 提交入口，完成玩家可参与的一环。
...
24. [ ] 人物入世闭环：完成 EMB01～EMB04 后，玩家可以自由代入/离开并提交每季一次决定；结果必须回到同一权势账、Fact、具体史事和 Situation，而非停留在独立角色面板。
25. [ ] 政治可见性纵切：完成 POL02～POL05，交付稳定派系、朝堂席位图、首都政治脉搏与有证据的地图权力根基；POL06～POL08 随 B05 和后续政治行为持续接入。
26. [ ] 入世身份扩展：按角色身份逐个完成 EMB05，EMB06～EMB08 持续作为裁决一致性、历史记录、生命周期、存档和防滥用门禁，不一次铺开所有身份动作。
...
v1.2.0 的 P0 演示目标已经达成：支持、请令、安抚与削权 Facts 已进入同一套具体场面和 POL01 权势资源账，人物、朝堂、局势、三问、本季变化和关注暂停使用同一口径。当前下一段是 EMB01～EMB04：允许玩家随时入世一名人物，每季提交一次同规则行动，再把结果送回现有闭环。
...
EMB01～EMB04 的演示目标是：玩家可从人物档案代入任一存活角色，在本季选择一项符合其处境的关系、机会或立场行动；推进后由同一模拟系统决定结果并写入此人历史，随后可以继续代入、换人或退出观察，全程不把角色变成国家控制器。
