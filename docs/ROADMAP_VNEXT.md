...
> 任务状态：Phase A 已完成；Phase B 已交付三类权威 Situation、事实型结案层与 B10 局势全卷，B09 数值化起止快照因权威字段缺失继续保留；Phase C 的 C01～C13 已完成 Situation-first 观察循环、人物记忆、跨季盘算，并交付“争取支持 → 请领军令 → 换帅/安抚/削权”的首段权威人物链。INF01 已以 v1.0.1 交付更新闭环，v1.1.0 是首个玩法 minor。LOOP01 总任务仍待 POL01、NAR 场面投影与 EMB 入世共用同一条链后关闭，不再把它们作为互不相干的功能铺开
...
22. [ ] 统一人物权力纵切：C12/C13 首段已完成支持、请令与机构回应；下一步把相同 Fact 接入 NAR01～NAR04 的具体场面、POL01 权势结果账，再由 EMB01～EMB04 共用同一 Intent/Resolver 提交入口。四组任务前后衔接，不能各自形成孤岛。
...
当前 P0 的演示目标是：把 v1.1.0 的支持、请令、安抚与削权 Fact 接入同一套具体场面和 POL01 权势资源账，使人物、朝堂与局势入口都能说明同一件事，而不是各自展示摘要或总值。
...
- 不立即铺开 B05/B07 等更多检测器；C12/C13 与 ARC01 已完成，当前把同一行动链接入 NAR01～NAR04、POL01 与 ARC02 后续，不提前铺开叛乱、派系夺权或海军对等行动。
