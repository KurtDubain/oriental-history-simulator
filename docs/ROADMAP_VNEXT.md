...
> 任务状态：Phase A 已完成；Phase B 已交付三类权威 Situation、事实型结案层与 B10 局势全卷，B09 数值化起止快照因权威字段缺失继续保留；Phase C 的 C01～C11 已完成 Situation-first 观察循环、人物记忆与跨季盘算，并让副将请领独立军令成为第一条权威行动链；下一纵切进入 C12/C13 军中支持与抗命/削权/安抚接管
...
20. [ ] 下一纵切：C12/C13，让军中支持成为可积累的前置行动，再用同一裁决契约接管抗命、削权与安抚；同时完成 ARC01 架构基线，并以 ARC02 的阶段接口承载新增行为，不再扩大 `engine.ts` 单体。
21. [ ] C14/C15 前置门：完成 ARC03/ARC05 的第一轮拆分，确保权力基础、叛乱和后续战争系统不会继续堆入 App Shell 或总校验文件。
22. [ ] 地图与档案扩展前置门：完成 ARC04/ARC06；ARC07 持续作为每次拆分的行为保持门，ARC08 仅在性能证据满足时启动。
...
下一轮的演示目标是：人物不只“等待支持值足够”，而会通过提携、军中关系与家族背书主动积累支持；朝廷与主将随后能以抗命、削权或安抚回应，但仍不提前接入叛乱。
...
- 不立即铺开 B05/B07 等更多检测器；下一轮只进入 C12/C13 的军中支持与抗命/削权/安抚接管，不提前铺开叛乱、派系夺权或海军对等行动。
