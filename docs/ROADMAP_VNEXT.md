...
> 任务状态：Phase A 已完成；Phase B 已交付三类权威 Situation、事实型结案层与 B10 局势全卷，B09 数值化起止快照因权威字段缺失继续保留；下一纵切进入 C01/C02，让当世三问稳定追踪 Situation，但暂不进入关注、自动暂停或人物 Agency
...
14. [ ] 下一纵切：优先 C01/C02，把当世三问切换为稳定 Situation 题源并加入连续性仲裁；不在此之前进入 Situation 关注、自动暂停或人物 Agency。

这一 Sprint 的 B 阶段演示目标已经达成：

> 同一场战争从宣战、会战、战线变化到议和或覆灭始终保持一个可追溯身份，玩家可在局势全卷中读懂并逐层查证这段历史。

下一轮的演示目标是：当世三问连续数季追踪同一条关键局势，除非出现更高优先级的明确结构转折，不因单季分数抖动换题。
...
- 不立即铺开 B05/B07 等更多检测器；下一轮先完成 C01/C02，让当世三问持续追踪已经可读的 Situation。
- 不先做完整围城 UI。
- 不先增加宗教、文化、资源、兵种或地图。
