...
> 任务状态：Phase A 已完成；Phase B 已交付三类权威 Situation、事实型结案层与 B10 局势全卷，B09 数值化起止快照因权威字段缺失继续保留；Phase C 的 C01～C11 已完成 Situation-first 观察循环、人物记忆与跨季盘算，并让副将请领独立军令成为第一条权威行动链；INF01 已以 v1.0.1 交付可辨识发布、游戏内检查更新与安全重载。LOOP01 已完成三主循环归属基线，下一步直接进入统一人物权力故事纵切，串联 NAR01～NAR04、C12/C13、POL01 与后续 EMB01～EMB04，不再把它们作为互不相干的功能分别铺开
...
22. [ ] 统一人物权力纵切：以 NAR01～NAR04 的具体史事为可理解层，以 C12/C13 的支持积累和朝廷回应为权威行动层，以 POL01 为权势结果账，并预留 EMB01～EMB04 共用同一 Intent/Resolver 的提交入口；四组任务必须前后衔接，不能各自形成孤岛。
23. [ ] 架构前置门：统一纵切同步完成 ARC01、ARC02，不再扩大 `engine.ts` 单体，不沿用不可解释的派系总值，也不为四组任务各造一条状态链。
