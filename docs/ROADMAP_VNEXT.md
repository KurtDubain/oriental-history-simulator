...
> 任务状态：Phase A 已完成；Phase B 的 B01/B02/B03/B04/B06/B08/B12 已落地，军权危机、继承危机与战争进程共用同一权威生命周期；下一入口仍在 Phase B，优先完成 B09/B10 结案摘要与可见详情页，不提前进入人物 Agency
...
13. [ ] 下一纵切：优先 B09/B10，把已有 Situation 做成可读、可追溯的结案摘要与详情页，再考虑增加第四种检测器。

这一 Sprint 的演示目标只有一句话：

> 同一场战争从宣战、会战、战线变化到议和或覆灭始终保持一个可追溯身份；下一步让玩家在界面上直接读懂这段历史。
...
- 不立即铺开 B05/B07 等更多检测器；下一轮先完成 B09/B10，让现有三类 Situation 真正可读。
- 不先做完整围城 UI。
- 不先增加宗教、文化、资源、兵种或地图。
