...
export interface PersonStoryBeatView {
  phase: 'command' | 'choice' | 'battle' | 'setback' | 'return' | 'ending';
  phaseLabel: string;
  dateLabel: string;
  title: string;
  summary: string;
...
function PersonInspector({ data, onOpenMind, mobileMindRequest = 0, ...actions }: Extract<InspectorProps, { kind: 'person' }> & { onOpenMind?: () => void; mobileMindRequest?: number }) {
  ...
  return (
    <>
      <div className="observer-inspector__header">
        <div className="observer-inspector__identity"><span className="observer-inspector__kind"><UserRound size={14} aria-hidden="true" />人物档案{actions.embodiment?.active ? <b className="observer-embodiment-seal">入世中</b> : null}</span><h2>{data.name}{data.courtesyName ? <small> 字{data.courtesyName}</small> : null}</h2><p>{[data.polity, data.role, data.lifeStage, `${data.age}岁`].filter(Boolean).join(' · ')}</p></div>
        <InspectorActions label={data.name} {...actions} />
      </div>
